...
import AlertEdit from "../alertEdit"; // Importe o modal de edição
...
interface AlertData {
    ...
    machine: string;
    description: string;
    status: "Não resolvido" | "Em progresso" | "Resolvido";
    checked: boolean;
}
...
function getStatusBadge(status: AlertData["status"]) {
    switch (status) {
        case "Não resolvido":
            return (
                <span className="bg-[#FFE0E5] text-[#E83838] border-[#E83838] border px-2 py-1 rounded text-xs">
                    Não resolvido
                </span>
            );
        case "Em progresso":
            return (
                <span className="bg-[#FAFAFA] text-[#C7A20D] border-[#C7A20D] border px-2 py-1 rounded text-xs">
                    Em progresso
                </span>
            );
        case "Resolvido":
            return (
                <span className="bg-[#C1E1B2] text-[#42AE10] border-[#42AE10] border px-2 py-1 rounded text-xs">
                    Resolvido
                </span>
            );
        default:
...
export default function AlertHistory() {
    ...
    const [selectedStatus, setSelectedStatus] = useState<"Não resolvido" | "Em progresso" | "Resolvido" | "Todos">("Todos");
    ...
    // Exemplo de seleção de checkbox
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4 ml-4">
                ...
                <div className="bg-[#616061] p-2 rounded-lg">
                    <button
                        ...
                        className={`px-3 py-1 rounded-lg ${selectedStatus === "Não resolvido" ? "bg-white text-[#444444]" : "bg-[#616061]"}`
                        }
                    >
                        Não resolvido
                    </button>
                    <button
                        ...
                        className={`px-3 py-1 rounded-lg ${selectedStatus === "Em progresso" ? "bg-white text-[#444444]" : "bg-[#616061]"}`
                        }
                    >
                        Em progresso
                    </button>
                    <button
                        ...
                        className={`px-3 py-1 rounded-lg ${selectedStatus === "Resolvido" ? "bg-white text-[#444444]" : "bg-[#616061]"}`
                        }
                    >
                        Resolvido
                    </button>
